...
public class Test {
    public static void main(String[] args) {
        ...
        Expression W =
                new Expression("W^{\\alpha}_{\\beta}=P^{\\alpha}_{\\beta}+(\\lambda/2)*R^\\alpha_\\beta");

        KINV.eval(new Expression("\\gamma=\\lambda/(1-\\lambda)"));
        K_2.eval(new Expression("\\lambda=0"));
        W.eval(new Expression("\\lambda=0"));
        KINV.eval(new Expression("\\lambda=0"));
